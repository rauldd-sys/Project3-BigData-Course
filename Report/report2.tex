...
\begin{figure}[p]
    \centering
    \includegraphics[width=\textwidth]{studio3T.png}
    \caption{Initial Studio 3T exploration of the \texttt{encounters} collection. The GUI provided a quick schema overview and spot-checked the first few documents before automating the analysis in code.}
    \label{fig:studio3t}
\end{figure}
\FloatBarrier
...
The MapReduce section mirrors the Hadoop reducers: the first job simply sums cases and deaths, while the second buffers (date, cases) pairs per country, sorts them, and scans a sliding 14-day window in a \texttt{finalize} function, following the pattern recommended in the MongoDB documentation \cite{MapReduceDatabaseManual}. Inline outputs take around 0.35 to 0.48~seconds on my machine.
...
\begin{figure}[p]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Playground command block}
    \end{subfigure}\hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{screenshot_mongo_results.png}
        \caption{Sample aggregation results}
    \end{subfigure}
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{screenshot_mongo_country_time.png}
        \caption{Country MapReduce timing}
    \end{subfigure}\hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{screenshot_mongo_rolling_time.png}
        \caption{Rolling-window timing}
    \end{subfigure}
    \caption{MongoDB playground evidence: loading the script, inspecting the aggregation output, and capturing the runtime for each MapReduce job.}
    \label{fig:mongo-playground}
\end{figure}
\FloatBarrier

\subsection{Hadoop Streaming}\label{sec:hadoop}
All scripts live in \texttt{Hadoop/}. The modernised version replaces the tutorial word-count with two project-specific jobs while following the job-setup pattern described in the official MapReduce tutorial \cite{ApacheHadoop3422025}.
\begin{itemize}[leftmargin=1.2cm]
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Country totals job submission}
    \end{subfigure}\hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{screenshot_hadoop_results.png}
        \caption{Reducer output sample}
    \end{subfigure}
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{screenshot_hadoop_2.png}
        \caption{Rolling-window job submission}
    \end{subfigure}\hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{screenshot_hadoop_2_results.png}
        \caption{Rolling-window results}
    \end{subfigure}
    \caption{Hadoop Streaming runs inside the Docker container: each command, together with the head of the HDFS output for the country totals and rolling 14-day incidence jobs.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{spark-submit invocation}
    \end{subfigure}\hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{screenshot_spark_results.png}
        \caption{Preview with runtime print-out}
    \end{subfigure}
    \caption{Spark CLI execution: the left panel shows the submission command, while the right panel confirms the top-country preview and reports the ~8 second total runtime emitted by the script.}
    \label{fig:spark-cli}
\end{figure}
\FloatBarrier
...
\noindent The full monthly animation is included in the submission (\path{Project3/visualizations/covid_animation.mp4}) and mirrored online at \href{https://github.com/rauldd-sys/Project3-BigData-Course/blob/bf0ccccac5b6f8c0a8112404d1aa68418e771dfa/visualizations/covid_cases.mp4}{this repository link}.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}p{3.5cm}p{4cm}p{4cm}@{}}
        ...
        \midrule
        MongoDB MapReduce & Country totals & 0.37 \\
        MongoDB MapReduce & Peak 14-day & 0.47 \\
