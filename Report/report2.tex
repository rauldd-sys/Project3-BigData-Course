...
MongoDB proved ideal for fast exploration and quick tweaks inside the playground. Hadoop demanded more scaffolding but delivered fine-grained control that mirrors the exercises from class. Spark ran the same aggregations with compact code and produced Parquet outputs, even if the JVM start-up kept runtimes a little higher—a ranking that aligns with independent benchmarks contrasting Spark and MapReduce on analytical workloads \cite{tekdoganBenchmarkingApacheSpark2021,vermaBigDataManagement2016}.
